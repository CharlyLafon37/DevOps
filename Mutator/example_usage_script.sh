...
# CONFIG:
BIN="./target/classes/mutation"
SPOON_JAR="./lib/spoon-core-5.5.0.jar"
...
#processor="$1"
processor="main.java.mutation.AtoBProcessor"
...
if ! javac -classpath "${SPOON_JAR}" -d "${BIN}"; then
echo;
echo "Error compiling, Spoon aborted.";

echo "javac -classpath ${SPOON_JAR} -d ${BIN}"
